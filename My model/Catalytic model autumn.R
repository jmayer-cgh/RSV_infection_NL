...
# A = mean FOI (proportion infected per day) for children born in autumn
...
# SIMULATION TIME  ---------------------------------------------------------
agepred <- autumn.df$agemid


# TEST MODEL  ---------------------------------------------------------

test <- model_au(theta, agepred, inits)
...
# Test function
loglik(theta, agepred, autumn.df, model_au, inits)
...
# save the trace
saveRDS(trace, "trace_FOI_w_autumn.rds")


# POSTPROCESSING AND RESULTS -----------------------------------

# Calculate simulated trajectory quantiles
trajsim <- maketrajsim(tracefinal, theta, agepred, model_au, inits, 1000)
