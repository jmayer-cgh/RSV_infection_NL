...
model_sp <- function(theta, age, inits) {
  ...
  catalytic <- function(age, state, param) {
    ...
    lambda = param[["P"]] + age * 0 #constant FOI for now, change 0 to something else later
...
theta <- c(P=0.02, B = 0.01) # these are just random values, to be fitted
...
# Test function
loglik(theta, agepred, spring.df, model_sp, inits)
...
# Estimated params
estpars <- c("P", "B") # parameters to estimate, can be modified
...
# save the trace
saveRDS(trace, "trace_FOI_w_spring.rds")


# POSTPROCESSING AND RESULTS -----------------------------------

# Calculate simulated trajectory quantiles
trajsim <- maketrajsim(tracefinal, theta, agepred, model_sp, inits, 1000)
