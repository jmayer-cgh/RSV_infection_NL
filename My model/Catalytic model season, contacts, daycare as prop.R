...
model <- function(theta, age, inits, data) {
  ...
  catalytic <- function(age, state, param, data) {
    ...
    dM_au = -mu*M_au
    dM_wt = -mu*M_wt
    dS_sp = + mu*M_sp - (1-daycare_sp)*lambda_sp*S_sp - daycare_sp*lambda_sp_d*S_sp
    ...
    dZ_wt = + (1-daycare_wt)*lambda_wt*S_wt + daycare_wt*lambda_wt_d*S_wt
    dZ_all = + (1-daycare_sp)*lambda_sp*S_sp + (1-daycare_sm)*lambda_sm*S_sm +
               (1-daycare_au)*lambda_au*S_au + (1-daycare_sp)*lambda_wt*S_wt +
              daycare_sp*lambda_sp_d*S_sp + daycare_sm*lambda_sm_d*S_sm + 
              daycare_au*lambda_au_d*S_au + daycare_wt*lambda_wt_d*S_wt
    ...
    return(list(c(dM_sp/M_sp,dM_sm/M_sm, dM_au/M_au,dM_wt/M_wt,
                ...
                lambda_sp_d=lambda_sp_d, lambda_sm_d = lambda_sm_d, 
                lambda_au_d = lambda_au_d, lambda_wt_d = lambda_wt_d,
                mu=mu))
