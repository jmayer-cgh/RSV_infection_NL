...
model_sm <- function(theta, age, inits) {
  ...
  catalytic <- function(age, state, param) {
    ...
    lambda = param[["U"]] + age * 0 #constant FOI for now, change 0 to something else later
...
# U = mean FOI (proportion infected per day) for children born in summer
# B = rate of waning maternal immunity
theta <- c(U=0.02, B = 0.01) # these are just random values, to be fitted
...
# SIMULATION TIME  ---------------------------------------------------------
agepred <- summer.df$agemid


# TEST MODEL  ---------------------------------------------------------

test <- model_sm(theta, agepred, inits)
...
# Test function
loglik(theta, agepred, summer.df, model_sm, inits)
...
# Estimated params
estpars <- c("U", "B") # parameters to estimate, can be modified
...
# save the trace
saveRDS(trace, "trace_FOI_w_summer.rds")


# POSTPROCESSING AND RESULTS -----------------------------------

# Calculate simulated trajectory quantiles
trajsim <- maketrajsim(tracefinal, theta, agepred, model_sm, inits, 1000)
