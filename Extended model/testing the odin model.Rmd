...
msr_test <- odin::odin("~/Library/CloudStorage/OneDrive-Charité-UniversitätsmedizinBerlin/LSTHM project/Extension/RSV_infection_NL/Extended model/Odin model.R")
...
case_compare <- function(state, observed, pars = NULL){
  ...
  R_sp <- state[4, , drop = T]
  R_sm <- state[8, , drop = T]
  R_au <- state[12, , drop = T]
  R_wt <- state[16, , drop = T]
  total_sero_modelled <- sum(R_sp, R_sm, R_au, R_wt)
  seroprevalence_observed <- observed$cum_infection
  lambda <- total_sero_modelled  +
    rexp(n = length(total_sero_modelled ), rate = exp_noise)
  dpois(x = seroprevalence_observed, lambda = lambda, log = T)
}
...
plot_particle_filter <- function(history, true_history, times, obs_end = NULL) {
  ...
  par(mar = c(4.1, 5.1, 0.5, 0.5), las = 1)
  cols <- c(M1 = "#8c8cd9", M2 = "turquoise", S = "#cc0044", R = "#999966")
  matplot(times, t(history[1, , -1]), type = "l",
          xlab = "Time", ylab = "Number of individuals",
          col = cols[["M1_sp"]], lty = 1, ylim = range(history))
  matlines(times, t(history[2, , -1]), col = cols[["M2_sp"]], lty = 1)
...
plot_particle_filter(filter$history(), true_history, incidence_data$age_days)
```
