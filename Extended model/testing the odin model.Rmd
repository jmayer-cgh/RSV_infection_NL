...
```{r}
palette <- c("M1" = "#F2862C", "M2" = "#cc0044", "S" = "#8c8cd9", "R" = "#999966", "R_all" = "pink")
...
case_compare <- function(state, observed, pars = NULL){
  ...
  prob <- state[18, , drop = T] / colSums(state[2:17, , drop = T]) # proportion seroconverted at each age point (model output)
...
prob <- x[18, , ] / colSums(x[2:17, , ]) # proportion seroconverted at each age point (model output)
# prob <- observed$prop_seroconv
...
plot_particle_filter <- function(history, true_history, times, obs_end = NULL) {
  ...
  matlines(times, t(history[18, , -1]), col = cols[["R_all"]], lty = 1)
