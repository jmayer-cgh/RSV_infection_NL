...
case_compare <- function(state, observed, pars = NULL){
 ...
 # prob <- observed$prop_seroconv
  ...
  prob_spring <- state[5, , drop = T] / colSums(state[2:5, , drop = T]) 
  ...
  prob_summer <- state[9, , drop = T] / colSums(state[6:9, , drop = T])
  ...
  prob_autumn <- state[13, , drop = T] / colSums(state[10:13, , drop = T])
  ...
  prob_winter <- state[17, , drop = T] / colSums(state[14:17, , drop = T]) 
...
plot_particle_filter <- function(history, true_history, times, obs_end = NULL) {
  ...
  matlines(times, t(history[17, , -1]), col = cols[["prop_sero_tot"]], lty = 1)
...
prop_seroconv <-  (state_single[17, , -1])
...
matlines(times, (state_single[17, , ]), lty = 1, col = cols["prop_sero_tot"])
legend("left", lwd = 1, col = cols, legend = names(cols), bty = "n")
...
write.csv(summary_hpd, paste0(path, "highest probability distribution, seasonal fit and mu.csv"), row.names = F)
write.csv(seroconversion_df, paste0(path, "seroconversion by age, seasonal fit and mu.csv"), row.names = F)
```
